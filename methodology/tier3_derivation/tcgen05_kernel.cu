// =====================================================================
//  tcgen05_kernel.cu — TMEM Double-Buffer + Stage-A Reorder Variant
//  (preserved for future optimization work; NOT the production kernel)
// =====================================================================
//
// STATUS as of 2026-04-20:
//   - Correctness: 128/128 PASS on Modal B200 (after GT-48 fix applied)
//   - Latency: mean 0.039 ms, p95 0.078 ms (matches baseline within noise;
//     no perf gain over flashinfer-bench-starter-kit/solution/cuda/kernel.cu)
//   - The intended H-reduce / MMA-pipe overlap was consumed by the GT-48
//     drain barrier (__syncthreads after wait_ld). The architectural
//     mechanism is sound but the practical win requires a lighter
//     drain primitive than __syncthreads.
//
// WHAT THIS KERNEL IS:
//   Tier 3 + Option C base, plus:
//     - TMEM double-buffer: alloc 128 cols, regions A=[base], B=[base+64]
//     - 2 mbarriers (mbar_A, mbar_B) with phase-parity tracking
//     - Stage A reorder: iter pidx dispatches MMA[next_pidx] BEFORE
//       waiting on MMA[curr_pidx], so MMA[next] hardware pipeline runs
//       concurrent with H-reduce of curr
//     - Race-fix: stage B (cp.async K[later] + scale write) moved AFTER
//       stage D-1 so scale_smem write doesn't race the curr-page scale read
//     - GT-48 fix: __syncthreads after tcgen05_wait_ld to drain the
//       tensor pipe before any subsequent op (especially stage B's K_smem
//       overwrite for the same buffer)
//
// FUTURE OPTIMIZATION IDEAS (motivated by why this kernel doesn't beat baseline):
//
// CANDIDATES A-F1 TESTED 2026-04-20 — none improved over the 0.039 ms variant:
//
// ┌─────┬───────────────────────────────────────────────┬──────────┬─────────────┐
// │ ID  │ Mechanism                                     │ Result   │ Latency     │
// ├─────┼───────────────────────────────────────────────┼──────────┼─────────────┤
// │ A   │ Move GT-48 drain past H-reduce                │ FAIL 59  │ —           │
// │ B   │ __threadfence_block instead of __syncthreads  │ FAIL 59  │ —           │
// │ C   │ 4 TMEM regions (alloc 256, no reuse)          │ PASS 128 │ 0.040 ms    │
// │ D   │ Named bar.sync 1 (different barrier ID)       │ PASS 128 │ 0.040 ms    │
// │ E   │ fence::before_thread_sync + __syncthreads     │ PASS 128 │ 0.040 ms    │
// │ F1  │ Subset bar.sync 1, 64 (warps 0-1 only)        │ FAIL 59  │ —           │
// │ —   │ baseline GT-48 fix (this kernel, as-is)       │ PASS 128 │ 0.039 ms    │
// │ —   │ production baseline (no TMEM2 reorder)        │ PASS 128 │ 0.038 ms    │
// └─────┴───────────────────────────────────────────────┴──────────┴─────────────┘
//
// REFINED GT-48 (from A-F1 investigation): the drain barrier is QUADRUPLY rigid:
//     (1) Position: must be IMMEDIATELY after wait_ld (Cand A failed)
//     (2) Function: must be bar.sync, not __threadfence_block (Cand B failed)
//     (3) Cost: any bar.sync variant has equivalent cost (Cand D, E confirmed)
//     (4) Scope: must be FULL-CTA (all 128 threads) — subset bar.sync fails (F1)
//   Per-warp-subset drain via named bar with thread count = 64 does NOT trigger
//   the SM-wide tensor pipe quiescence. This closes the split-warp design path
//   (Cand F2: parallel work for warps 2-3 during drain) since we cannot decouple
//   warp groups for tensor pipe purposes — they must all participate together.
//
// No fix-mechanism variant we've tried recovers the H-reduce/MMA-pipe overlap.
//
// CANDIDATES STILL UNTESTED (require external tooling or fundamental rework):
//   G: NCU profiling on Modal (CLI works, output is text/CSV). Counters to
//      query: sm__inst_executed_pipe_tensor.*, smsp__warp_issue_stalled_*,
//      sm__warps_active.tcgen05_*. May reveal the SPECIFIC pipe stall the
//      drain introduces, suggesting a non-obvious workaround.
//   H: Try a DIFFERENT MMA shape (M=128 instead of M=64) — would change
//      tensor pipe scheduling pattern. Risky, would need rederivation.
//   I: Cluster + DSM (sm_100a) — could share TMEM across CTA-cluster, may
//      avoid per-CTA tensor pipe drain. GT-44 noted clusters regressed
//      previously but with different design; might be worth retry.
//   J: Eliminate the Stage A reorder entirely — if the H-reduce/MMA-pipe
//      overlap can't be recovered, the reorder costs more than it saves.
//      Would mean reverting tcgen05_kernel.cu to baseline structure (no
//      reorder, no double-buffer). This is essentially baseline.
//
// REGRESSION TEST: any change to this kernel MUST be validated against
//   diagnostic_tests/tmem2_p11_kmem_overwrite_race.cu (the GT-48 detector).
//   Run on Modal B200 with B=32 multi-CTA. If P11c shows K_C contamination,
//   the change has reintroduced the GT-48 race.
//
// REGRESSION TEST: any change to this kernel MUST be validated against
//   diagnostic_tests/tmem2_p11_kmem_overwrite_race.cu (the GT-48 detector).
//   Run on Modal B200 with B=32 multi-CTA. If P11c shows K_C contamination,
...
//
// PROVENANCE:
//   - Forked from flashinfer-bench-starter-kit/solution/cuda/kernel.cu
//     (GT-43 baseline at 0.038 ms) on 2026-04-19
//   - Stage A reorder added 2026-04-19 (failed, see GT-47)
//   - Race-fix on stage B (move after D-1) added 2026-04-20 (still failed)
//   - 6-step bisection 2026-04-20 isolated bug to Stage A reorder (GT-47)
//   - P6-P11 probes 2026-04-20 identified root cause as tcgen05.commit's
//     mbar firing before tensor pipe drain (GT-48)
//   - GT-48 fix applied 2026-04-20: __syncthreads after wait_ld
//   - 128/128 PASS verified 2026-04-20 (Modal app brw1bbs9h)
//
// =====================================================================
//
// Original Tier 3 + Option C description (preserved for context):
//   Fused tcgen05 FP8 MMA + SMEM-transpose H-reduction.
...
// Opt-TMEM2: TMEM double-buffer adds a 2nd mbarrier slot (8 B); WEIGHTS_SMEM offset shifts.
...
constexpr int MBAR_A_SLOT_OFFSET  = 41760;               // 8 B (page-region A)
constexpr int MBAR_B_SLOT_OFFSET  = 41768;               // 8 B (page-region B)
constexpr int WEIGHTS_SMEM_OFFSET = 41792;               // 256 B ([H=64] FP32) — 16-B aligned
constexpr int SMEM_BYTES          = 42048;
...
// Opt-TMEM2: alloc 128 TMEM cols for double-buffered MMA destinations.
// Region A = [tmem_col, tmem_col+64), Region B = [tmem_col+64, tmem_col+128).
...
// CANDIDATE C: alloc 256 cols (4 regions × 64) for no-reuse design.
    ...
    int N_max)
{
    ...
    // mbar slots A/B accessed by shared-space addr below (not via C++ ptr)
    ...
    // Opt-TMEM2: two mbarriers indexed by (page_index & 1).
    const uint32_t mbar_s_buf[2] = {
        __cvta_generic_to_shared(smem_raw + MBAR_A_SLOT_OFFSET),
        __cvta_generic_to_shared(smem_raw + MBAR_B_SLOT_OFFSET) };

    // ---- P1: TMEM alloc (warp 1) — 128 cols for double-buffered MMA destinations ----
    if (warp_id == 1) {
        tcgen05_alloc_128(alloc_slot_s);
    }
    __syncthreads();
    const uint32_t tmem_col_base = alloc_slot_ptr[0];   // base; region A here, B at +64
    const uint32_t tmem_col_buf[2] = { tmem_col_base, tmem_col_base + 64u };
    ...
    // ---- P3: mbarrier init (warp 0, threads 0 and 1 each init one) ----
    if (warp_id == 0 && lane_id == 0) {
        mbarrier_init_1(mbar_s_buf[0]);
    }
    if (warp_id == 0 && lane_id == 1) {
        mbarrier_init_1(mbar_s_buf[1]);
    }
    // ---- P3b: weights cooperative load (parallel with mbar init) — Option C ----
    if (tid < H) {
        weights_smem[tid] = weights[b * H + tid];
    }
    uint32_t phase_buf[2] = { 0, 0 };
    __syncthreads();
    ...
    // ---- Page loop (Opt-2 K double-buffer + Opt-TMEM2 TMEM double-buffer) ----
    // Opt-TMEM2 restructure: dispatch MMA[next_page] BEFORE the heavy work for
    // curr_page, so MMA[next] hardware pipeline runs concurrent with H-reduce[curr].
    // Each page uses TMEM region (page_index & 1) and mbarrier (page_index & 1).
    //
    // Per-iter ordering (after a 2-page prologue):
    //   A. issue MMA[next_pidx] -> commit -> mbar[next_pidx & 1]   (if has_next)
    //   C. wait MMA[curr_pidx]  -> tcgen05.ld + wait::ld           (consumes K_smem[curr & 1])
    //   B. cp.async K[later_pidx] into K_smem[curr & 1] + scale     (only AFTER C: was racing pre-C)
    //   D. scale apply + scores_smem + __syncthreads + H-reduce + GMEM write
    //   E. cp_async_wait_all + __syncthreads                        (if has_later: visibility)
    //
    // Per-token zero-fill removed (GT-43 #7): MMA output is column-independent.
    const int num_active = min(PAGES_PER_CTA,
                               max(0, max_num_pages - cta_page_start));

    if (num_active > 0) {
        // ---- Prologue: async-load page 0 K (and page 1 K if exists) + scales ----
        // Both pages must be visible before issuing MMA[0] so MMA[1] can dispatch
        // immediately in iter 0's stage A.
        {
            const int32_t phys0 = bt_cache_smem[0];
            ...
            if (num_active >= 2) {
                const int32_t phys1 = bt_cache_smem[1];
                const size_t  pg_base1 = size_t(phys1) * PAGE_BYTES;
                #pragma unroll
                for (int i = 0; i < 4; ++i) {
                    int u        = tid * 4 + i;
                    int tok      = u >> 3;
                    ...
                    int k_base   = k_uint4 << 4;
                    int off      = smem_8xT_offset(tok, k_base);
                    cp_async_16B(k_shared_buf[1] + off,
                                 k_cache + pg_base1 + tok * INDEX_HEAD_DIM + k_base);
                }
                if (tid < PAGE_SIZE) {
                    scale_smem_buf[1][tid] = *reinterpret_cast<const float*>(
                        k_cache + pg_base1 + PAGE_DATA_BYTES + tid * 4);
                }
            }
            cp_async_commit();
        }
        ...
        for (int pidx = 0; pidx < num_active; ++pidx) {
            ...
            // ---- STAGE A: issue MMA[next_pidx] into TMEM region (next_pidx & 1) ----
            // Reads K_smem[next_pidx & 1], visible from prior iter's stage E (or prologue).
            if (has_next) {
                if (warp_id == 0 && elect_one_sync()) {
                    const int next_tmem  = next_pidx & 1;
                    const int next_K_buf = next_pidx & 1;
            ...
            // ---- STAGE C: wait for curr MMA, read TMEM region (curr_tmem) ----
            ...
            tcgen05_wait_ld();
            // GT-48 FIX (KEEP — empirically minimum cost variant, QUADRUPLY rigid):
            // tcgen05.commit's mbarrier arrival fires BEFORE the tensor pipe has fully
            // drained. This bar.sync triggers tensor-pipe quiescence. Tested 2026-04-20:
            //   ✗ A: move past H-reduce        → FAIL 59/128 (drain must be RIGHT here)
            //   ✗ B: __threadfence_block        → FAIL 59/128 (must be bar.sync)
            //   ~ C: 4 TMEM regions + drain    → PASS @ 0.040 (no perf win, more SMEM)
            //   ~ D: bar.sync 1 (named)        → PASS @ 0.040 (any bar.sync triggers drain)
            //   ~ E: fence::before + sync      → PASS @ 0.040 (fence redundant)
            //   ✗ F1: bar.sync 1, 64 (subset)  → FAIL 59/128 (must be FULL CTA)
            // Conclusion: __syncthreads is the empirical best (0.039 ms / 128/128 PASS).
            __syncthreads();

            // ---- STAGE D-1: scale apply + scores_smem (reads scale_smem_buf[curr & 1]) ----
            // Opt-#5: row stride = SCORES_STRIDE = 65 (breaks 16-way bank conflict).
            // NOTE: scale_smem_buf[curr & 1] MUST be read BEFORE stage B overwrites it.
            // The __syncthreads below orders all scale reads (lane<16 of every warp) before
            // stage B's scale writes (tid<64 — a disjoint thread set).
            ...
            // Barrier: scores_smem writes visible to all H-reduce threads
            // AND scale-apply reads complete before stage B's scale writes.
            __syncthreads();

            // ---- STAGE D-2: SMEM-transpose H-reduction + GMEM write ----
            // Runs concurrent with MMA[next] hardware pipeline (issued in stage A).
            ...
            // ---- STAGE B: cp.async K[later] into K_smem[curr & 1] + write scale[later] ----
            // Moved AFTER stage D-1 so scale_smem_buf[curr & 1] write doesn't race the
            // curr-page scale read. K_smem[curr & 1] was consumed by MMA[curr] (mbar wait
            // in C confirmed completion); now safe to overwrite.
            if (has_later) {
                const int later_K_buf = later_pidx & 1;   // == curr_tmem (pidx & 1)
                const int32_t phys_l  = bt_cache_smem[later_pidx];
                const size_t  pg_base_l = size_t(phys_l) * PAGE_BYTES;
                #pragma unroll
                for (int i = 0; i < 4; ++i) {
                    int u        = tid * 4 + i;
                    int tok      = u >> 3;
                    ...
                    int k_base   = k_uint4 << 4;
                    int off      = smem_8xT_offset(tok, k_base);
                    cp_async_16B(k_shared_buf[later_K_buf] + off,
                                 k_cache + pg_base_l + tok * INDEX_HEAD_DIM + k_base);
                }
                cp_async_commit();
                if (tid < PAGE_SIZE) {
                    scale_smem_buf[later_K_buf][tid] = *reinterpret_cast<const float*>(
                        k_cache + pg_base_l + PAGE_DATA_BYTES + tid * 4);
                }
            }

            // ---- STAGE E: visibility for next iter's stage A MMA descriptor ----
            // Next iter pidx+1's stage A dispatches MMA[pidx+2] reading K_smem[(pidx+2) & 1]
            // = K_smem[curr_tmem] = the buffer stage B just wrote.
            if (has_later) {
                cp_async_wait_all();
                __syncthreads();
            }
    ...
    torch::Tensor topk_indices)
{
    ...
    // Phase 4+5 (Tier 2(c) + Opt-#3 + Opt-#4): adaptive top-K + fused fill.
