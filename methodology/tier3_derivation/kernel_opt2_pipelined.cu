...
constexpr int SCORES_SMEM_OFFSET  = 24576;               // 16384 B ([64 rows × 64 cols] FP32)
constexpr int SCALE0_SMEM_OFFSET  = 40960;               // 256 B
constexpr int SCALE1_SMEM_OFFSET  = 41216;               // 256 B
constexpr int BT_CACHE_OFFSET     = 41472;               // 16 B
constexpr int ALLOC_SLOT_OFFSET   = 41488;               // 16 B
constexpr int MBAR_SLOT_OFFSET    = 41504;               // 16 B
constexpr int WEIGHTS_SMEM_OFFSET = 41520;               // 256 B ([H=64] FP32)
constexpr int SMEM_BYTES          = 41776;
    ...
    int N_max)
{
    ...
    // ---- Page loop (Opt-2: K double-buffered software pipeline) ----
    // Overlap: next page's K+scale load runs concurrent with current page's MMA.
    // Tail-safe via num_active; loads into buffer (pidx+1)&1 happen only if (pidx+1) < num_active.
    const uint4 zero_uint4 = make_uint4(0u, 0u, 0u, 0u);
    const int num_active = min(PAGES_PER_CTA,
                               max(0, max_num_pages - cta_page_start));

    if (num_active > 0) {
        // ---- Prologue: load page 0 into buffer 0 ----
        {
            const int pidx0    = 0;
            const int page_id0 = cta_page_start + pidx0;
            const int32_t phys0 = bt_cache_smem[pidx0];
            const size_t pg_base0 = size_t(phys0) * PAGE_BYTES;
            uint8_t* Kb = K_smem_buf[0];
            float*   Sb = scale_smem_buf[0];
            #pragma unroll
            for (int i = 0; i < 4; ++i) {
                int u        = tid * 4 + i;
                int tok      = u >> 3;
                ...
                int global_t = page_id0 * PAGE_SIZE + tok;
                int off      = smem_8xT_offset(tok, k_base);
                if (global_t >= seq_len_b) {
                    *reinterpret_cast<uint4*>(Kb + off) = zero_uint4;
                } else {
                    uint4 val = *reinterpret_cast<const uint4*>(
                        k_cache + pg_base0 + tok * INDEX_HEAD_DIM + k_base);
                    *reinterpret_cast<uint4*>(Kb + off) = val;
                }
            }
            if (tid < PAGE_SIZE) {
                int global_t = page_id0 * PAGE_SIZE + tid;
                Sb[tid] = (global_t >= seq_len_b) ? 0.0f
        ...
        for (int pidx = 0; pidx < num_active; ++pidx) {
            ...
            // ---- Overlap: start loading next page's K+scale into next_buf ----
            // Runs on all 128 threads concurrent with MMA dispatch; MMA reads
            // K_smem_buf[curr_buf] only, so writes to K_smem_buf[next_buf] are safe.
            if (has_next) {
                const int page_id_n   = cta_page_start + next_pidx;
                const int32_t phys_n  = bt_cache_smem[next_pidx];
                const size_t pg_base_n = size_t(phys_n) * PAGE_BYTES;
                uint8_t* Kn = K_smem_buf[next_buf];
                float*   Sn = scale_smem_buf[next_buf];
                #pragma unroll
                for (int i = 0; i < 4; ++i) {
                    int u        = tid * 4 + i;
                    int tok      = u >> 3;
                    int k_uint4  = u & 7;
                    int k_base   = k_uint4 << 4;
                    int global_t = page_id_n * PAGE_SIZE + tok;
                    int off      = smem_8xT_offset(tok, k_base);
                    if (global_t >= seq_len_b) {
                        *reinterpret_cast<uint4*>(Kn + off) = zero_uint4;
                    } else {
                        uint4 val = *reinterpret_cast<const uint4*>(
                            k_cache + pg_base_n + tok * INDEX_HEAD_DIM + k_base);
                        *reinterpret_cast<uint4*>(Kn + off) = val;
                    }
                }
                if (tid < PAGE_SIZE) {
                    int global_t = page_id_n * PAGE_SIZE + tid;
                    Sn[tid] = (global_t >= seq_len_b) ? 0.0f
                        : *reinterpret_cast<const float*>(
            ...
            // Barrier: scores_smem reads done (next iter MMA overwrites TMEM, reuses same scores_smem)
            __syncthreads();
        }
    }
...
//  One CTA per batch. 512 threads × 16 items/thread = 8192 capacity (covers
//  all dataset N_max up to 5760). CUB BlockRadixSort + BLOCK_LOAD_WARP_TRANSPOSE
//  for coalesced loads. Output BLOCKED → thread t holds ranks [t*16, (t+1)*16).
// =====================================================================
    ...
    int max_num_pages)
{
    ...
    // Load keys (STRIPED coalesced GMEM -> BLOCKED registers).
    ...
    // Build BLOCKED values = position indices matching the BLOCKED key layout.
    ...
    // Sort descending. Input BLOCKED; output BLOCKED (thread t holds ranks t*16..(t+1)*16-1).
    BlockRadixSortT(temp_storage.sort).SortDescending(keys, vals);

    // Extract top-K and translate in one pass.
    #pragma unroll
    for (int i = 0; i < TOPK_ITEMS_PER_THREAD; ++i) {
        ...
        if (rank < actual_topk) {
            ...
            topk_indices[b * TOPK + rank] = phys * PAGE_SIZE + (lid & 63);
        }
    }
}
    ...
    torch::Tensor topk_indices)
{
    ...
    // Phase 4+5 (fused, Tier 2(c)): block radix-sort top-K + translate in one kernel.
