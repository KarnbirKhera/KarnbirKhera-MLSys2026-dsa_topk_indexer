...
// Tier 3: fused tcgen05 FP8 MMA replaces Tier 2 dequant_gather + cuBLAS bmm.
// Per-page iteration: FP8 K from HBM -> SMEM 8xT (GT-11) -> tcgen05.mma.kind::f8f6f4
// -> TMEM accumulator -> tcgen05.ld -> register scale multiply -> SMEM stage -> GMEM.
// Downstream (weighted_relu_sum, at::topk, translate_indices) unchanged from Tier 2.
...
constexpr int SCORES_SMEM_OFFSET = 16384;               // 16384 B
...
constexpr int SMEM_BYTES         = 33072;
...
// =====================================================================
//  Fused FP8 MMA scoring kernel (Tier 3)
//  Grid:  (ceil(max_num_pages / PAGES_PER_CTA), B)
//  Block: 128 threads (4 warps)
//  SMEM:  33 KiB
// =====================================================================
__global__ __launch_bounds__(128, 2)
void fp8_mma_scores_kernel(
    ...
    float*         __restrict__ scores_all,    // [B, 64, N_max]
    int max_num_pages,
    int N_max)
{
    ...
    for (int pidx = 0; pidx < PAGES_PER_CTA; ++pidx) {
        ...
        // ---- P15: cooperative GMEM store scores_smem -> scores_all ----
        // 64 rows x 64 cols = 4096 FP32. 128 threads x 32 iters = 4096 writes.
        // Thread tid writes to (m, n) where flat = i*128 + tid, m = flat/64, n = flat%64.
        {
            float* out_base = scores_all
                            + size_t(b) * 64 * N_max
                            + size_t(page_id) * PAGE_SIZE;  // start column
            #pragma unroll
            for (int i = 0; i < 32; ++i) {
                int flat = i * 128 + tid;
                int m = flat >> 6;       // / 64
                int n = flat & 63;       // % 64
                out_base[m * N_max + n] = scores_smem[m * MMA_N + n];
            }
        }
        __syncthreads();
    }
    ...
    torch::Tensor topk_indices)
{
    ...
    // Phase 1+2 (fused): FP8 MMA producing [B, 64, N_max] FP32 scores.
    auto scores_all = torch::empty({B, 64, N_max}, opts_f32);

    const int grid_x = (max_num_pages + PAGES_PER_CTA - 1) / PAGES_PER_CTA;
    dim3 grid(grid_x, B);
    fp8_mma_scores_kernel<<<grid, 128, SMEM_BYTES, stream>>>(
        q_ptr, k_cache_ptr, bt_ptr, sl_ptr,
        scores_all.data_ptr<float>(),
        max_num_pages, N_max);
    AT_CUDA_CHECK(cudaGetLastError());

    // Phase 3: batched fused relu * weights * sum-over-heads.
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("launch_topk_c", &launch_topk_c, "DSA TopK FP8 indexer (Tier 3 tcgen05)");
}
